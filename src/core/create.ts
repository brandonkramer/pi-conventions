...
import { inferConventionsConfig, type ConventionsProjectLanguage } from "./infer.ts";
import { pathExists } from "./path.ts";

export type ConventionsPreset = "rust" | "typescript" | "go" | "python";
...
const SCHEMA_SOURCE_PATH = path.join(PACKAGE_ROOT, "schemas", "conventions.schema.json");
const PRESET_SOURCE_PATHS: Record<ConventionsCreateTarget, string> = {
  ...
  typescript: path.join(PACKAGE_ROOT, "examples", "conventions.typescript.json"),
...
export function parseCreateTargetAlias(value: string | undefined): ConventionsCreateTarget | undefined {
...
export function describeScaffoldResult(result: ScaffoldConventionsResult): string {
...
async function scaffoldFromPreset(
  ...
  await confirmOverwriteIfNeeded(ctx, targetDir, [targetConfigPath, targetSchemaPath]);
...
async function scaffoldFromGeneratedConfig(
  ...
  await confirmOverwriteIfNeeded(ctx, targetDir, [targetConfigPath, targetSchemaPath]);
  await mkdir(targetDir, { recursive: true });
  await writeFile(targetConfigPath, `${JSON.stringify(configDocument, null, 2)}\n`, "utf8");
  await copyFile(SCHEMA_SOURCE_PATH, targetSchemaPath);
}
...
async function confirmOverwriteIfNeeded(
  ...
  if (!ctx.hasUI) {
    throw new Error(`Refusing to overwrite existing conventions files in ${targetDir} without UI confirmation.`);
  }
