...
export type ConventionsProjectLanguage = "rust" | "typescript" | "go" | "python";
...
const FILE_FORBIDDEN_NAMES = ["util", "utils", "helper", "helpers", "common", "misc"];
...
export async function inferConventionsConfig(projectRoot: string): Promise<InferredConventionsConfig> {
  ...
  const languageFiles = inventory.files.filter(
    (file) => file.family === language && sourceRoots.some((root) => file.path.startsWith(root)),
  );
  const layerPrefixes = buildLayerPrefixes(sourceRoots, language, inventory);
  const usedFileNames = new Set(languageFiles.map((file) => file.stem.toLowerCase()));
    ...
    config: {
      ...
      policies: {
        structure: {
          ...
          newTopLevelFiles: buildTopLevelFileRule(language, sourceRoots, languageFiles, observedExtensions),
        },
        naming: {
          mode: "warn",
          editMode: "warn",
          rules: [
            ...fileRules,
            {
              ...
              forbiddenNames: DIRECTORY_FORBIDDEN_NAMES.filter((name) => !usedDirectoryNames.has(name)),
...
async function collectRepoInventory(projectRoot: string): Promise<RepoInventory> {
...
async function hasProjectMarker(projectRoot: string, fileName: string): Promise<boolean> {
...
function detectSourceRoots(
  ...
  const preferredRoots = ROOT_CANDIDATES[language]
    .filter((candidate) => containsLanguageFileUnder(candidate, language, inventory))
...
function containsLanguageFileUnder(
  ...
  return inventory.files.some((file) => file.family === language && file.path.startsWith(prefix));
}
  ...
  for (const root of sourceRoots) {
    ...
    const children = inventory.directories.filter((directory) => {
      ...
      return inventory.files.some(
        (file) => file.family === language && file.path.startsWith(`${directory}/`),
      );
    });
...
function buildTopLevelFileRule(
  ...
  return {
    ...
    mode: "block" as const,
    allowedFiles,
    extensions: observedExtensions.length > 0 ? observedExtensions : LANGUAGE_EXTENSIONS[language],
  };
}
...
function buildFileNamingRules(
  ...
  const extensions = observedExtensions.length > 0 ? observedExtensions : LANGUAGE_EXTENSIONS[language];
  const groupedByCase = new Map<NamingCaseStyle, string[]>();

  for (const extension of extensions) {
    const names = languageFiles.filter((file) => file.extension === extension).map((file) => file.stem);
    const caseStyle = detectDominantCaseStyle(names) ?? DEFAULT_CASE_STYLE[language];
    groupedByCase.set(caseStyle, [...(groupedByCase.get(caseStyle) ?? []), extension]);
  }
  ...
  return [...groupedByCase.entries()].map(([caseStyle, caseExtensions]) => ({
    ...
    forbiddenNames: FILE_FORBIDDEN_NAMES.filter((name) => !usedFileNames.has(name)),
