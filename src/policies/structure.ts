...
const DEFAULT_TOP_LEVEL_EXTENSIONS = ["rs", "go", "ts", "tsx", "mts", "cts", "d.ts"];
...
export function normalizeStructurePolicy(
  ...
  const forbiddenSegments = uniqueStrings(candidate.forbiddenSegments, (value) => value.toLowerCase());
  const layers = (candidate.layers ?? [])
    .map((layer): StructureLayer | undefined => {
      ...
      return {
        ...
        description: typeof layer.description === "string" ? layer.description.trim() : undefined,
        prefixes,
      };
    })
  ...
  const legacyZones = (candidate.legacyZones ?? [])
    .map((zone): StructureLegacyZone | undefined => {
      ...
      if (prefixes.length === 0 || typeof zone.reason !== "string" || zone.reason.trim().length === 0) {
        return undefined;
      }
      return {
        prefixes,
  ...
  const topLevelExtensions = uniqueStrings(topLevelRule.extensions, (value) => value.toLowerCase());
  ...
  return {
    ...
    forbiddenSegments:
      forbiddenSegments.length > 0 ? forbiddenSegments : DEFAULT_FORBIDDEN_SEGMENTS,
    layers,
    legacyZones,
    newTopLevelFiles: {
      enabled: topLevelRule.enabled ?? true,
      mode: parseMode(topLevelRule.mode, mode),
      allowedFiles: new Set(uniqueStrings(topLevelRule.allowedFiles, normalizeRelativePath)),
      extensions:
        topLevelExtensions.length > 0 ? topLevelExtensions : DEFAULT_TOP_LEVEL_EXTENSIONS,
    },
    notes,
  };
}
...
export function evaluateStructureViolation(
  ...
  if (config.newTopLevelFiles.enabled && isNewTopLevelSourceFile(relativePath, config)) {
    return {
      policyId: "structure",
...
export function buildStructurePromptLines(config: StructurePolicyConfig): string[] {
  const lines = [
  ...
  if (config.newTopLevelFiles.enabled) {
    ...
    if (allowedRootFiles.length > 0) {
      lines.push("", `Allowed top-level source files: ${allowedRootFiles.join(", ")}.`);
    }
    lines.push(
      "",
...
function isUnderSourceRoot(relativePath: string, config: StructurePolicyConfig): boolean {
...
function findLegacyZone(
  ...
  return config.legacyZones.find((zone) => zone.prefixes.some((prefix) => relativePath.startsWith(prefix)));
}

function isNewTopLevelSourceFile(relativePath: string, config: StructurePolicyConfig): boolean {
  for (const root of config.sourceRoots) {
    ...
    return extension !== undefined && config.newTopLevelFiles.extensions.includes(extension);
  }
  return false;
}
