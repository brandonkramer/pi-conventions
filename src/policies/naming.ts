...
export interface NamingRule {
	id?: string;
	exclude: string[];
	excludeMatchers: import("../core/pattern.ts").PathPattern[];
	prefixes: string[];
...
export function normalizeNamingPolicy(
	...
	const rules = (candidate.rules ?? [])
		.map((rule): NamingRule | undefined => {
			...
			const reason =
				typeof rule.reason === "string" ? rule.reason.trim() : undefined;
...
export function evaluateNamingViolation(
	...
	for (const rule of config.rules) {
		const prefix = rule.prefixes.find((candidate) =>
...
function parseCaseStyle(value: unknown): NamingCaseStyle | undefined {
	return typeof value === "string" &&
		VALID_CASE_STYLES.includes(value as NamingCaseStyle)
