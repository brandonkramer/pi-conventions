...
const EXTENSION_ENTRY_PATH = fileURLToPath(
	...
	return (pi: ExtensionAPI) => {
		pi.registerCommand("probe-check", {
			...
			handler: async () => {
				await appendFile(
					logPath,
					`${JSON.stringify({ event: "probe-command" })}\n`,
					"utf8",
				);
			},
		});
...
describe("SDK integration", () => {
	it("loads the package extension into a real session", async () => {
		...
		try {
			const { extensionsResult } = await createTestSession(
				cwd,
				agentDir,
				logPath,
			);
	...
	it("routes extension slash commands through session.prompt", async () => {
		...
		try {
			...
			expect(await readProbeLog(logPath)).toEqual([{ event: "probe-command" }]);
		} finally {
			await removeTempDir(agentDir);
			await removeTempDir(cwd);
		}
	...
	it("executes /conventions create rust in a real session", async () => {
		...
		try {
			...
			const config = await readJson<any>(
				path.join(cwd, ".pi", "conventions.json"),
			);
	...
	it("executes /conventions create documentation in a real session", async () => {
		const cwd = await createTempDir("pcg-sdk-documentation-");
		const agentDir = await createTempDir("pcg-sdk-agent-");
		const logPath = path.join(agentDir, "probe-log.jsonl");
		...
		try {
			...
			await session.prompt("/conventions create documentation");

			const config = await readJson<any>(
				path.join(cwd, ".pi", "conventions.json"),
			);
			expect(config.policies.documentation.mode).toBe("warn");
			expect(config.policies.documentation.rules[0].kind).toBe(
	...
	it("executes /conventions create fallback in a real session", async () => {
		...
		try {
			...
			const fallbackConfig = await readJson<any>(
