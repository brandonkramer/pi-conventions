...
describe("create helpers", () => {
	it("exposes create completions and aliases", () => {
		...
		expect(parseCreateTargetAlias("docs")).toBe("documentation");
		...
		expect(getConventionsCommandArgumentCompletions("create d")).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ value: "create documentation" }),
			]),
		);
	});
	...
	it("copies a shipped preset into the project .pi directory", async () => {
		...
		try {
			...
			expect(
				await readJson(path.join(repo, ".pi", "conventions.json")),
			).toMatchObject({
				policies: { documentation: { mode: "warn" } },
			});
		} finally {
	...
	it("writes the fallback config under the pi agent directory", async () => {
		...
		try {
			...
			expect(result).toMatchObject({
				kind: "fallback",
				targetDir: path.join(home, ".pi", "agent"),
			});
			expect(
				await readJson(path.join(home, ".pi", "agent", "conventions.json")),
			).toMatchObject({
